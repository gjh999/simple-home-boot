...
```
JDK 17  : C:\eGovFrame\eGovCI-5.0.0-Windows-64bit\bin\jdk-17.0.17+10
...
# 컴파일 검증
/c/eGovFrame/eGovCI-5.0.0-Windows-64bit/bin/apache-maven-3.9.9/bin/mvn clean compile -q

# 서버 실행
/c/eGovFrame/eGovCI-5.0.0-Windows-64bit/bin/apache-maven-3.9.9/bin/mvn spring-boot:run \
...
> 비밀번호는 이중 SHA-256 해시(SKILL.md 6-6 참조)로 저장됨
...
### KRDS 디자인 표준
- 한국 정부 통합 디자인 가이드(KRDS) 적용, Bootstrap 5 기반
- 정적 리소스: 로컬 파일 사용 (CDN 사용 금지)
  - `/static/css/krds.css`, `/static/js/krds.js`
  - `/static/css/bootstrap-icons.min.css` (폰트: `/static/css/fonts/`)
