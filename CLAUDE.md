...
src/main/resources/
  ...
  static/                 # 정적 리소스 (CSS/JS/이미지 — 공식 KRDS 킷 + 호환 레이어, 로컬·CDN 금지)
    krds/                 # 공식 KRDS HTML Component Kit (krds.min.css·Pretendard GOV 등)
    css/krds-compat.css   # Bootstrap 그리드/유틸 → KRDS 토큰 재구현(호환 레이어)
    css/krds.css          # KRDS 토큰 정규화·보정
    css/bootstrap-icons.min.css  # 아이콘만 유지(bi-*)
    js/krds-compat.js·krds.js·common.js
  templates/              # Thymeleaf 템플릿
    layouts/default.html  # 업무 레이아웃
    layouts/public.html   # 공개 레이아웃(랜딩·/krds-sample)
...
- ✅ **현황(2026-06-23)**: 메뉴·푸터·KRDS 예시·전 업무화면(게시판·일정·회원·로그인 등) **다국어 전수 적용 완료**. `message-ui_{ko,en}.properties` 키 집합 정합(동일 키), 하드코딩 한글 fallback 0건. 신규/변경 시 이 정합을 깨지 말 것.
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-06 | **KRDS 전면 적용**: Bootstrap 완전 제거 → 공식 KRDS 킷 전환(랜딩·대시보드·게시판·일정·회원·로그인 KRDS 네이티브). 공개 레이아웃(`layouts/public`) 도입, `/krds-sample` 컴포넌트 예시 페이지 추가, 페이지네이션·GNB KRDS 정합 |
| 2026-06 | **다국어(i18n) 전수 적용**: 메뉴·푸터·KRDS 예시·전 업무화면 메시지 키화, ko/en 키 정합, `/cmm/lang` 전환. 푸터 전자정부표준프레임워크 공식 홈페이지 배너, 전 페이지 '맨 위로' 버튼 |
