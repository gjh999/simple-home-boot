...
로그인하면 상단 **전역 내비게이션(GNB)** 과 함께 업무 기능을 이용하며, **권한(관리자/사용자)에 따라 메뉴가 달라집니다.**

**포털 대시보드(`/portal`)** — 환영 패널 · 공지/자유게시판 요약 · 바로가기(공지·일정·회원·API 문서)
![포털 대시보드](Docs/screenshots/portal.png)

**게시판 목록(`/bbs/{bbsId}/list`)** — KRDS 표 · 검색(구분/검색어) · 페이지네이션 · 글쓰기
![게시판 목록](Docs/screenshots/board-list.png)

**로그인 후 이용 가능한 기능**

| 구분 | 기능 |
| :--- | :--- |
| 공통(로그인) | 포털 대시보드 · **게시판**(목록·상세·작성·수정·삭제, 첨부파일) · **일정관리**(등록·조회·수정·삭제) · **마이페이지**(내 정보·비밀번호) · 다국어(한/영) |
| 관리자(ROLE_ADMIN) | **게시판 마스터**(게시판 정의) · **게시판 사용정보** · **회원관리**(목록·상세·수정) · 관리자 비밀번호 변경 |

> KRDS 적용 상세는 [Docs/krds-적용-가이드.md](Docs/krds-적용-가이드.md), 자체검증은 `.claude/skills/krds-conversion/scripts/krds-verify.sh` 참조.
> 스크린샷은 관리자(`admin`) 로그인 기준입니다.
